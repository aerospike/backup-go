...
func (w *Writer) validate(ctx context.Context) error {
	...
	if _, err := w.bucketHandle.Attrs(ctx); err != nil {
		return fmt.Errorf("%w: failed to get bucketHandler %s attributes: %w", models.ErrNotFound, w.bucketName, err)
	}
...
func (w *Writer) Remove(ctx context.Context, targetPath string) error {
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("%w: failed to read object attributes from bucket %s: %w", models.ErrStorage, w.bucketName, err)
		}
		...
		if err = w.bucketHandle.Object(objAttrs.Name).Delete(ctx); err != nil {
			return fmt.Errorf("%w: failed to delete object %s: %w", models.ErrStorage, objAttrs.Name, err)
		}
	}
...
func (c *crcWriter) Close() error {
	...
	if local != remote {
		...
		if err := c.objectHandle.Delete(c.ctx); err != nil {
			return fmt.Errorf("%w: checksum mismatch: %d != %d and failed to delete object: %w", models.ErrCorruptData,
				local, remote, err)
		}

		return fmt.Errorf("%w: checksum mismatch: %d != %d", models.ErrCorruptData, local, remote)
	}
