...
func NewWriter(ctx context.Context, opts ...options.Opt) (*Writer, error) {
	...
	if w.IsRemovingFiles {
		...
		if err != nil {
			return nil, fmt.Errorf("%w: failed to remove files: %w", models.ErrStorage, err)
		}
	}
...
func createDirIfNotExist(path string, isDir bool) error {
	...
	if err := os.MkdirAll(path, 0o700); err != nil {
		return fmt.Errorf("%w: failed to create directory: %w", models.ErrStorage, err)
	}
...
func isEmptyDirectory(path string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("%w: failed to read path %s: %w", models.ErrStorage, path, err)
	}
...
func (w *Writer) Remove(ctx context.Context, targetPath string) error {
	...
	if w.WithNestedDir {
		...
		if err := os.RemoveAll(targetPath); err != nil {
			return fmt.Errorf("%w: failed to remove targetPath %s: %w", models.ErrStorage, targetPath, err)
		}
	...
	if err != nil {
		...
		if isNotDir(err) { // it's a file, remove a single file
			if err = os.Remove(targetPath); err != nil && !errors.Is(err, os.ErrNotExist) {
				return fmt.Errorf("%w: failed to remove file %s: %w", models.ErrStorage, targetPath, err)
			}

			return nil
		}

		return fmt.Errorf("%w: failed to open %s: %w", models.ErrStorage, targetPath, err)
	}
	...
	f, err := root.Open(".")
	if err != nil {
		return fmt.Errorf("%w: failed to open root directory %s: %w", models.ErrStorage, targetPath, err)
	}
	defer f.Close()

	files, err := f.ReadDir(-1)
	if err != nil {
		return fmt.Errorf("%w: failed to read root directory %s: %w", models.ErrStorage, targetPath, err)
	}
	...
	for _, file := range files {
		...
		if err = root.Remove(file.Name()); err != nil {
			...
			return fmt.Errorf("%w: failed to remove file %s: %w", models.ErrStorage, file.Name(), err)
		}
	}
...
func (w *Writer) NewWriter(ctx context.Context, filename string) (io.WriteCloser, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: failed to prepare backup directory: %w", models.ErrStorage, err)
	}
	...
	case w.IsDir:
		...
		if err != nil {
			return nil, fmt.Errorf("%w: failed to open root %s: %w", models.ErrStorage, w.PathList[0], err)
		}
		defer root.Close()

		file, err := root.OpenFile(filename, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0o600)
		if err != nil {
			return nil, fmt.Errorf("%w: failed to open file %s in root %s: %w", models.ErrStorage, filename, w.PathList[0], err)
		}
	...
	case !w.IsDir && filename != "":
		...
		if err != nil {
			return nil, fmt.Errorf("%w: failed to open root %s: %w", models.ErrStorage, dir, err)
		}
		defer root.Close()

		file, err := root.OpenFile(filename, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0o600)
		if err != nil {
			return nil, fmt.Errorf("%w: failed to open file %s in root %s: %w", models.ErrStorage, filename, dir, err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("%w: failed to open file %s: %w", models.ErrStorage, filePath, err)
	}
