...
func runBackupRestoreDirectory(suite *backupRestoreTestSuite,
	...
	fmt.Println("=====COMPPARING===", len(expectedRecs), "=========", statsBackup.GetReadRecords())
	...
	suite.Require().Equal(uint64(0), statsRestore.GetRecordsSkipped())
	suite.Require().Equal(uint64(0), statsRestore.GetRecordsFresher())
	suite.Require().Equal(uint64(0), statsRestore.GetRecordsExisted())
	suite.Require().Equal(uint64(0), statsRestore.GetRecordsIgnored())
...
func (suite *backupRestoreTestSuite) TestRestoreExpiredRecords() {
	...
	data := &bytes.Buffer{}
	encoder := backup.NewEncoder[*models.Token](backup.EncoderTypeASB, "test", false)
