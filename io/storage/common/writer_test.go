...
func TestGetFullPath(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "path list traversal rejected",
			prefix:   "backup",
			filename: "",
			pathList: []string{"../escape/file.bin"},
			isDir:    false,
			want:     "",
			wantErr:  true,
			errMsg:   "object key must not contain '..' path segments",
		},
		{
			name:     "nested object key allowed",
			prefix:   "backup",
			filename: "folder/file.asb",
			pathList: []string{"data/records.bin"},
			isDir:    true,
			want:     "backup/folder/file.asb",
			wantErr:  false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := GetFullPath(tt.prefix, tt.filename, tt.pathList, tt.isDir)
...
func TestGetFullPath_Consistency(t *testing.T) {
	...
	result1, err1 := GetFullPath(prefix, filename, pathList, false)
	result2, err2 := GetFullPath(prefix, filename, pathList, false)
